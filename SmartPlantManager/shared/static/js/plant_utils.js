function formatPlant(p) {
    ...
    if (t != null && th.temp_min !== undefined && th.temp_max !== undefined) {
        if (t < th.temp_min || t > th.temp_max) 
            tStatus = 'alarm';
        else if (t < th.temp_min * 1.05 || t > th.temp_max * 0.95) 
            tStatus = 'warning';
    }

    if (h != null && th.humidity_min !== undefined && th.humidity_max !== undefined) {
        if (h < th.humidity_min || h > th.humidity_max) 
            hStatus = 'alarm';
        else if (h < th.humidity_min * 1.05 || h > th.humidity_max * 0.95) 
            hStatus = 'warning';
    }

    if (s != null && th.soil_min !== undefined && th.soil_max !== undefined) {
        if (s < th.soil_min || s > th.soil_max) 
            sStatus = 'alarm';
        else if (s < th.soil_min * 1.1 || s > th.soil_max * 0.9) 
            sStatus = 'warning';
    }
    ...
    if (batt != null) {
        if (batt < 10) { 
            score -= 10; bStatus = 'alarm'; 
        }
        else if (batt < 20) { 
            score -= 5; bStatus = 'warning'; 
        }
    }
