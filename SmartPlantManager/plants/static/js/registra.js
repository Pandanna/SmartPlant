...
async function validaStep1(isManual) {
    ...
    const deviceId = deviceInput.value.trim().toLowerCase();
    ...
    reader.onload = ev => {
        ...
        img.onload = () => {
            ...
            const updatePreview = (pfx) => {
                ...
                if (img) 
                    img.src = jpegDataUrl; img.style.display = 'block';
...
function goToStep(n) {
    ...
    if (n === 2) {
        selectedDevice = document.getElementById('device-id-input').value.trim().toLowerCase();
...
async function startAnalysis() {
    ...
    try {
        ...
        if (contentType && contentType.indexOf("application/json") !== -1) {
            ...
            // Se non è JSON, probabilmente è una pagina di errore di Django
...
async function submitManual() {
    ...
    if (!common) 
        return alert('Inserisci il nome della pianta.');
...
function showResult(data) {
    ...
    // forza manualmente l'ultimo pallino del progresso come 'fatto'
