...
from plants.models import Dispositivo
...
def admin_utenti(request):
    utenti = Utente.objects.annotate(num_piante=Count('piante')).order_by('created_at')
...
def admin_crea_dispositivo(request):
    if request.method == 'POST':
        device_id = request.POST.get('device_id', '').strip().lower()
